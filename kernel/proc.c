	...
	for(int i = 0; i < NMMAP; i++){
		...
		if(vma->start > 0){
			...
			for(uint64 vm0 = PGROUNDDOWN(vma->start); vm0 < vma->end; vm0 += PGSIZE){
				...
				char *mem = kalloc();
				if(mem == 0){
					munmap(np, vma->start, vma->end - vma->start);
					return -1;
				}
...
exit(int status)
{
  ...
  for(int fd = 0; fd < NOFILE; fd++){
	...
	for(int i = 0; i < NMMAP; i++){
		struct vmarea *vma = &p->vma[i];
		if(vma->start > 0){
			if(munmap(p, vma->start, vma->end - vma->start) < 0){
				panic("exit: munmap");
			}
		}
	}
...
int
mmap(struct proc *p, uint64 addr){
	for(int i = 0; i < NMMAP; i++){
		struct vmarea *vma = &p->vma[i];
		
		//cycle through unused vma
		if(vma->start == 0){
			continue;
		}
		
		//cycle through incorrect vmas
...
munmap(struct proc *p, uint64 addr, uint length){
	for(int i = 0; i < NMMAP; i++){
		...
		for(uint64 vm0 = PGROUNDDOWN(addr); vm0 < addr+length; vm0 += PGSIZE){
			...
			//The dirty bit (D) in the RISC-V PTE indicates whether a pagetable has been modified
			...
			if(!((vm0 < addr && vma->start < addr) || (vm0 + PGSIZE > addr+length && addr+length < vma->end))){
				kfree((void*)pa0);
				*pte = 0;
			}
		}
		...
		//If munmap removes all pages of a previous mmap, decrement reference count of file
